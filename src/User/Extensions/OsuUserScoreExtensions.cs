...
namespace DragonFruit.Orbit.Api.User.Extensions
{
    public static class OsuUserScoreExtensions
    {
        ...
        /// <param name="limit">Optional maximum entries to return </param>
        /// <param name="includeFails">Whether to include fails when requesting recent scores</param>
        public static IEnumerable<OsuScore> GetUserScores<T>(this T client, OsuUserCard user, UserScoreType type, string mode, uint page = 0, uint? limit = null, bool includeFails = false)
            where T : OrbitClient
        {
            return client.GetUserScores(user.Id, type, mode, page, limit, includeFails);
        }
        ...
        /// <param name="limit">Optional maximum entries to return </param>
        /// <param name="includeFails">Whether to include fails when requesting recent scores</param>
        public static IEnumerable<OsuScore> GetUserScores<T>(this T client, uint id, UserScoreType type, string mode, uint page = 0, uint? limit = null, bool includeFails = false)
            where T : OrbitClient
        {
            var request = new OsuUserScoresRequest(id, type)
            {
